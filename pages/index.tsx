...
export default function Home() {
  const postManagerContract = "0x0a13017eb770509a8f19e26e134ca8ee2c100a9c"; //postManager smart contract address
  ...
  async function getAllPosts() {
    ...
    if (ethereum) {
      ...
      const allPosts = await postManagerContractInstance.getPostsData(allPostsAddresses)
    ...
    try {
      ...
      if (noOfPosts === 0) {
        ...
        if (ethereum) {
          ...
          let { hash } = await postManagerContractInstance.createPost(cid,imageCid,filename,
            {
              gasLimit: 1200000,
            }
          );
  ...
  async function postComment(postData: PostDetail) {
    try {
      ...
      if (ethereum) {
        ...
        let { hash } = await postManagerContractInstance.addComment(newCid,postData.postSCAddress,{gasLimit: 1200000,});
